...
interface ReplicaSetFormProps {
  replicaSet?: V1ReplicaSet | null;
  onChange?: (payload: any) => void;
  isEdit?: boolean;
}

export const ReplicaSetForm = ({ 
  replicaSet, 
  onChange,
  isEdit = false 
}: ReplicaSetFormProps): JSX.Element => {
  ...
  useEffect(() => {
    if (replicaSet) {
      setName(replicaSet.metadata?.name || '');
      setNamespace(replicaSet.metadata?.namespace || 'default');
      
      // Convert labels to array format
      if (replicaSet.metadata?.labels) {
        const labelArray = Object.entries(replicaSet.metadata.labels).map(([key, value]) => ({ key, value }));
      ...
      if (replicaSet.metadata?.annotations) {
        const annotationArray = Object.entries(replicaSet.metadata.annotations)
      ...
      if (replicaSet.spec) {
        ...
        const container = replicaSet.spec.template?.spec?.containers?.[0];
        if (container) {
      ...
      <div className='px-4 space-y-6'>
        <div>
          ...
          <Field>
            ...
            <Description>
              {isEdit 
                ? 'The replica set name cannot be changed.' 
                : 'Enter a unique name for your replica set.'}
            </Description>
            <Input 
              name="name" 
              value={name} 
              onChange={(event) => setName(event.target.value)} 
              placeholder={isEdit ? undefined : "e.g., my-replicaset"}
              disabled={isEdit}
          ...
          <Field>
            ...
            <Description>
              {isEdit 
                ...
                : 'Select the namespace for this replica set.'}
            </Description>
            <NamespaceSelect 
              value={namespace} 
        ...
        <div>
          ...
          <Field>
            ...
            <Description>
              Name for the container (optional, defaults to replica set name).
            </Description>
            <Input 
              name="containerName" 
              value={containerName} 
